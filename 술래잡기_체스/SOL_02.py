""" 술래잡기 체스 / 20260924 / 체감 난이도: G3
소요 시간 1시간 37분 / 시도 2회 / 실행 시간 62ms (코드트리) / 메모리 17MB (코드트리)

이건 왜 다시 풀어도 어려운거지 ㅋㅋ 나중에 또 풀어보자.
백트래킹 함수 안에 집어넣어야 할 기능이 너무 많고, 배열 복사도 신경써야 해서 어렵게 느껴지는 것 같다.
1) 루틴을 확실히 따랐고, 이제는 입력값 받자마자 디버그 프린트 함수부터 짜서 확인하며 진행하고 있다.
2) 전반적인 접근 방법은 이전과 거의 동일한데, 코드 자체는 훨씬 정돈된 것이 눈이 띈다.
3) 이전엔 도둑말을 상수시간으로 찾기 위해 정보를 꾸준히 업데이트 했다.
4) 이번엔 4*4 배열 전체순회가 부담스럽지 않다는 점을 명확히 인지하고, 오히려 에러 위험이 큰 요소들을 최대한 제하는 방식으로 구현을 했다.
5) 이번에도 실수가 하나 있었는데, 역시나 덮어쓰기류였고, 브레이크 포인트 잘 찍어둔 덕에 빠르게 디버깅했다.
"""

def debug_board(arr, msg):
    if DEBUG_MODE:
        print(msg)
        print('UNIQUE ID       | DIRECTIONS')
        for cr in range(4):
            for cc in range(4):
                print(f'{arr[cr][cc][0]:2d}', end=' ')
            print(' ' * 4 + '|', end=' ')
            for cc in range(4):
                print(arr[cr][cc][1], end=' ')
            print()
        print('---')


def find_runner(n, arr):
    for rr in range(4):
        for rc in range(4):
            if arr[rr][rc][0] == n:
                rd = arr[rr][rc][1]
...
def move_runners(cr, cc, arr):
    for runner_id in range(1, 17):
        rr, rc, rd = find_runner(runner_id, arr)

        if rr < 0:
            continue

        for turn in range(8):
            current_dir = (rd+turn) % 8
            nr = rr + dr[current_dir]
            nc = rc + dc[current_dir]
            ...
            if nr == cr and nc == cc:
                continue
            if arr[nr][nc][0] >= 0:
                other_id = arr[nr][nc][0]
                other_dir = arr[nr][nc][1]
                arr[nr][nc][0] = runner_id
                arr[nr][nc][1] = current_dir
                arr[rr][rc][0] = other_id
                arr[rr][rc][1] = other_dir
                break
    return arr


def get_targets(cr, cc, cd, arr):
    result = []
    while True:
        cr += dr[cd]
        cc += dc[cd]
...
def backtrack(cr, cc, arr, sm=0, step=0):
    # TODO 1.1: 술래 정보 업데이트
    #   술래의 방향 정보를 위치 기반으로 설정
    #   잡은 도망자의 번호만큼 점수 추가
    #   보드에서 (cr, cc)의 식별자와 방향을 0, 0으로 비우기
    sm += arr[cr][cc][0]
    cd = arr[cr][cc][1]
    arr[cr][cc][0] = 0
    arr[cr][cc][1] = -100

    # TODO 1.2 도망자 정보 업데이트
    arr = move_runners(cr, cc, arr)

    # TODO 1.3: 목표물 탐색
    #   get_targets에서 다음 목표 리스트 받기
    targets = get_targets(cr, cc, cd, arr)

    # TODO 1.4 조기 종료
    #   술래가 갈 수 있는 칸이 없으면 종료
    #   최고 점수 업데이트
    global answer
    if not targets:
        if sm > answer:
    ...
    for nr, nc in targets:
        arr_copy = [[y[:] for y in x] for x in arr]
        backtrack(nr, nc, arr_copy, sm, step+1)
...
if __name__ == '__main__':
    ...
    for r in range(4):
        ...
        for c in range(4):
            board[r][c][0] = line[c*2]
            board[r][c][1] = line[c*2+1]-1
