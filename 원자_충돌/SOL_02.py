""" 원자 충돌 / 20260918 / 체감 난이도: G5
소요 시간 43분 / 시도 1회 / 실행 시간 259ms (코드트리) / 메모리 27MB (코드트리)

엥 소요 시간이 조금이지만 늘어났네? 이전하고 굉장히 비슷하게 풀긴했는데, 이번엔 클래스 메서드를 활용해봤다.
"""
...
class Atom:
    ...
    def from_string(cls, string):
        x, y, m, s, d = list(map(int, string.split()))
        return Atom(x-1, y-1, m, s, d)

    @classmethod
    def get_four(cls, r, c, m, s, diag):
        if diag:
            return [
                Atom(r, c, m, s, 1),
                Atom(r, c, m, s, 3),
    ...
    def move(self):
        nr = (self.r + self.s * Atom.DR[self.d]) % N
        nc = (self.c + self.s * Atom.DC[self.d]) % N
        self.r, self.c = nr, nc
        return nr, nc


def merge(r, c, lst):
    mass_sum = sum([at.m for at in lst])
    speed_sum = sum([at.s for at in lst])
    direc_sum = sum([at.d % 2 for at in lst])

    if mass_sum < 5:
        return []
    elif direc_sum == 0 or direc_sum == len(lst):
        return Atom.get_four(r, c, mass_sum//5, speed_sum//len(lst), False)
    else:
        return Atom.get_four(r, c, mass_sum//5, speed_sum//len(lst), True)
...
if __name__ == '__main__':
    ...
    for _ in range(K):
        ...
        for key, value in counter.items():
            if len(value) < 2:
                nxt_atoms.extend(value)
            else:
                nxt_atoms.extend(merge(*key, value))
