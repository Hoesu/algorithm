""" 보도블럭 / 20260920 / 체감 난이도: S1
소요 시간 34분 / 시도 2회 / 실행 시간 73ms (코드트리) / 메모리 17MB (코드트리)

아 1 이상 차이나면 보도블럭 설치가 아예 불가능한 것을 누락했다.
장장 3시간에 걸친 회전 빙하 수난기로 인해 집중력이 저하된 것으로 보임.
다음 문제는 정신 똑바로 차리고 풀자.

그래도 전체적인 구현은 훨씬 간결해진 것이 보인다.
쓸데없는 집합이나 불리언 값 사용 없이 그냥 인덱싱을 활용해 직관적으로 해결했다.
"""
...
def check(lst):
    ...
    for i in range(len(lst)-1):
        ...
        # 다음 값이 현재 값보다 클 때: i번째 포함 이전 L개 값 체크
        elif lst[i] < lst[i+1]:
            for j in range(L):
                if i-j < 0:
                    return False
        ...
        # 다음 값이 현재 값보다 작을 때: i+1번째 포함 다음 L개 값 체크
        elif lst[i] > lst[i+1]:
            for j in range(L):
                if i+j+1 >= len(lst):
                    return False
