""" 격자 숫자 놀이 / 20260919 / 체감 난이도: S1
소요 시간 27분 / 시도 1회 / 실행 시간 98ms (코드트리) / 메모리 18MB (코드트리)

타이레놀 한알 먹고 두통을 억누른 뒤 풀었다 ㅎ
아이디어 자체는 동일한데, 실행부 코드가 많이 다듬어진게 보인다.
그리고 이번에는 종료 조건에서 실수하지 않았다.
"""
    ...
    while True and time <= 100:
        ...
        for r in range(len(arr)):
            ...
            for k, v in sorted(counter.items(), key=lambda x: [x[1], x[0]]):
                temp.append(k)
                temp.append(v)
            max_length = min(max(max_length, len(temp)), 100)
            if len(temp) > 100:
                arr[r] = temp[:100]
            else:
                arr[r] = temp
        ...
        # 제로 패딩 추가
        for r in range(len(arr)):
            if len(arr[r]) < max_length:
                padding = [0] * (max_length-len(arr[r]))
                arr[r] = arr[r] + padding
