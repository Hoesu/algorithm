""" 불안한_무빙워크 / 20260916 / 체감 난이도: S1
소요 시간 24분 / 시도 2 / 실행 시간 72ms (코드트리) / 메모리 17MB (코드트리)

다시 봐도 얄미운 문제... 꼴도 보기 싫었다.
이번에도 한번 틀렸는데, 무빙워크 회전하면 마지막 칸 자동으로 도달할 때 빼줘야 하는거 깜빡했다!!
그래도 이전 구현은 인덱싱 해야 하는데 deque 쓰고 난리 났었는데, 이번엔 그냥 배열로만 쉽게 짰다.
과거에 비해 함수화가 없어도 분기 처리를 잘 소화하게 된 것 같다.
"""
