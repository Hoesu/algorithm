""" 회전하는 빙하 / 20260920 / 체감 난이도: G3
소요 시간 3시간 / 시도 1회 / 실행 시간 360ms / 메모리 23MB (코드트리)

와 진짜 회전만 나오면 애가 왤케 저능해지는건지 모르겠다.
사실 이번 문제 오래 걸린 이유는 배열 카피를 만들지 않고 패턴을 활용해서 회전을 시켜보고 싶었다.
저번에 공부한 좌표 변환으로 배열 회전하는거랑 살짝 개념이 다르다.
나머지 파트는 그냥 할만했다. DFS 생각 안나서 잠시 멈칫하긴함.
무조건 다시 풀어볼 문제인것 같다.
"""
...
def rotate(level):
    ...
    for sr in range(0, l, p):
        for sc in range(0, l, p):

            for i in range(q):
                for j in range(q):

                    temp = arr[sr+i][sc+j]
                    arr[sr+i][sc+j] = arr[sr+i+q][sc+j]
                    arr[sr+i+q][sc+j] = arr[sr+i+q][sc+j+q]
                    arr[sr+i+q][sc+j+q] = arr[sr+i][sc+j+q]
                    arr[sr+i][sc+j+q] = temp


def melt():
    delta = [[0]*(2**N) for _ in range(2**N)]
    for cr in range(2**N):
        for cc in range(2**N):
            if arr[cr][cc] == 0:
                continue
    ...
    for cr in range(2**N):
        for cc in range(2**N):
            arr[cr][cc] = max(0, arr[cr][cc]+delta[cr][cc])
