""" 방화벽 설치하기 / 20260914 / 체감 난이도: G5
소요 시간 21분 / 시도 2회 / 실행 시간 372ms (코드트리) / 메모리 24MB (코드트리)

종료 조건 리턴문 까먹어서 시간초과 한번 발생했다. 쩝.
암튼 뭐 그냥 이전이랑 비슷하게 풀었는데, 코드는 훨씬 정돈된 듯.
"""
from collections import deque


def backtrack(lst, start=0, step=0):
    global answer
    if step == 3:
        unburnt = bfs(blocks+lst)
        if unburnt > answer:
            answer = unburnt
        return
    for i in range(start, len(empty)):
        backtrack(lst+[empty[i]], i+1, step+1)


def bfs(all_blocks: list):
    dr = [-1, 1, 0, 0]
    dc = [0, 0, -1, 1]
    que = deque()
    blk = [[0]*C for _ in range(R)]
