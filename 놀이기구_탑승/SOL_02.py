""" 놀이기구 탑승 / 20260918 / 체감 난이도: S5
소요 시간 20분 / 시도 1회 / 실행 시간 49ms (코드트리) / 메모리 16MB (코드트리)

딕셔너리 밸류 셋으로 해서 in 연산 별로 걱정 없는것? 말고는 별 차이 없어 보인다. 그냥 한번에 쭉 써봤다.
"""
        ...
        for cr in range(N):
            for cc in range(N):
                ...
                for i in range(4):
                    ...
                    if seats[nr][nc] == 0:
                        empty += 1
                    if seats[nr][nc] in table[pid]:
                        friends += 1

                candidates.append([friends, empty, cr, cc])
        candidates.sort(key=lambda x: [-x[0], -x[1], x[2], x[3]])
        seats[candidates[0][2]][candidates[0][3]] = pid
    ...
    for cr in range(N):
        for cc in range(N):
            ...
            for i in range(4):
                ...
                if not (0 <= nr < N and 0 <= nc < N):
                    continue
                if seats[nr][nc] in table[seats[cr][cc]]:
                    friends += 1
            score += points[friends]
    print(score)
