"""2048 게임 / 20260922 / 체감 난이도: G4
소요 시간 25분 / 시도 1회 / 실행 시간 265ms (코드트리) / 메모리 24MB (코드트리)

회전을 확실히 익혀두니까 참으로 요긴하다.
다른 각도로 회전시키면서 중력을 오른쪽으로만 흐르게 하면 4방향을 다 체크할 수 있다.
또한, 버퍼로 복잡하게 하지 않고 포인터로 구현해봤다.
어려워 보였는데, 의외로 훨씬 직관적으로 느껴졌다.
"""
...
def rotate(arr, idx):
    if idx == 0:
        ...
        return [list(x[::-1]) for x in zip(*arr)]
    elif idx == 2:
...
def gravitate(arr):
    ...
    for r in range(N):
        ...
        while origin:
            ...
            if value == 0:
                continue
            if destin[pointer] == value:
                destin[pointer] += value
                pointer -= 1
            else:
                if destin[pointer] == 0:
                    destin[pointer] += value
        ...
        arr[r] = destin
        if destin[pointer] > 0:
            non_zeroes += (N-pointer)
        else:
            non_zeroes += (N-1-pointer)
    return arr, non_zeroes
...
if __name__ == '__main__':
    ...
    # TODO: 정답 = max(다섯번 회전 후 최대값)
    answer = 0

    # TODO: 백트래킹으로 모든 회전 방식에 대한 길이 5의 중복 가능한 순열 추출
    permutations = []
    backtrack([])

    # TODO: 모든 순열과, 순열 내 회전 인덱스 순회
    #   인덱스 맞춰서 회전 적용하고, 오른쪽으로 중력 적용.
    #   5번 회전 후, 최대값 계산하여 answer 변수와 비교
    #   중간에 값 하나 밖에 안남으면 조기 종료도 가능하지 않을까?
    for permutation in permutations:
        temp = [x[:] for x in board]

        # 회전 인덱스 순회
        for rotation_idx in permutation:
            temp = rotate(temp, rotation_idx)
            temp, count = gravitate(temp)
