""" 토스트_계란틀 / 20260917 / 체감 난이도: S1
소요 시간 15분 / 시도 1회 / 실행 시간 221ms (코드트리) / 메모리 24MB (코드트리)

그냥 BFS 문제. 예전에 풀었을 땐 내가 되게 기발하다 생각했었는데, 지금은 그냥 맨날하는거라 잘 모르겠다.
차이점을 뽑지면, 같은 그룹 구하고 새로운 달걀 값 구해서 바로 적용해주면 되는데, 굳이 굳이 딕셔너리 썼던 모습이 보인다.
"""
...
def bfs(sr, sc):
    ...
    while que:
        ...
        for i in range(4):
            ...
            if vst[nr][nc] != 0:
                continue
            if not(L <= abs(eggs[nr][nc]-eggs[cr][cc]) <= R):
                continue

            vst[nr][nc] = 1
            que.append((nr, nc))
            egg_sum += eggs[nr][nc]
            egg_coords.append((nr, nc))

    new_egg = egg_sum // len(egg_coords)
    for er, ec in egg_coords:
        eggs[er][ec] = new_egg
