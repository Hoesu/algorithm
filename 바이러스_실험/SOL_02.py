""" 바이러스 실험 / 20260920 / 체감 난이도: S2
소요 시간 29분 / 시도 1회 / 실행 시간 1645ms (코드트리) / 메모리 25MB (코드트리)

푼지 꽤 됐는데도 불구하고 개인적으로 기억에 남는 문제라 쉽게 풀었다.
근데 이번엔 3차원 배열 말고 딕셔너리로 풀면 어떻게 되는지 시간 비교를 해봤다.
결과적으로 시간이 훨씬 오래 걸렸는데, 어차피 양분 보너스 더할 때 전체순회가 불가피 하므로
3차원 배열로 처음부터 전체 순회하면서 시체 양분과 보너스 추가 동시에 더해주는게 훨 빠르다.
"""
if __name__ == '__main__':
    ...
    for _ in range(K):
        ...
        # 시체 배열
        corpses = [[0] * N for _ in range(N)]
        # 증식 배열
        splits = []

        # 바이러스 양분 섭취
        for (r, c), value in viruses.items():
            value.sort()
            survived = []

            for i in range(len(value)):
                # 양분 섭취 못하면 죽고 추가 영양분 생성.
                if value[i] > nutrients[r][c]:
                    corpses[r][c] += value[i] // 2
                # 자기 나이만큼 양분 섭취하면 나이 1 증가.
                else:
                    nutrients[r][c] -= value[i]
                    survived.append(value[i] + 1)

                    # 나이가 5의 배수면 증식 대상에 추가
                    div, mod = divmod(value[i] + 1, 5)
                    if div >= 1 and mod == 0:
                        splits.append((r, c))

            # 살아남은 바이러스로 갈아 끼우기
            viruses[(r, c)] = survived
        ...
        for cr in range(N):
            for cc in range(N):
                nutrients[cr][cc] += bonus[cr][cc]
                nutrients[cr][cc] += corpses[cr][cc]

    # 정답 출력
    answer = 0
    for value in viruses.values():
        answer += len(value)
    print(answer)
